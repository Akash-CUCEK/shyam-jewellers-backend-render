...
public class TokenBlacklistService {
  ...
  public void blacklistToken(String token, long expiryInSeconds) {
    ...
    // In practice, we might not want to hash the token again because the token is already a JWT and
    // we have the raw token.
    // But to be consistent with the previous Redis version (which stored the raw token), we store
    // the raw token.
    ...
    blacklistedToken.setTokenHash(
        token); // storing the raw token (not hashed) for simplicity and to match previous behavior
