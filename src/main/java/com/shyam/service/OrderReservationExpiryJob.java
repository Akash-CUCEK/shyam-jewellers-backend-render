...
/**
 * Scheduled job to expire pending orders that have been pending for more than 15 minutes.
 * Releases the reserved quantity for all order items in such orders.
 */
@Slf4j
@Component
...
public class OrderReservationExpiryJob {
    ...
    public void expirePendingOrders() {
        ...
        List<Order> pendingOldOrders = orderRepository.findByStatusAndCreatedAtLessThan(OrderStatus.PENDING, fifteenMinutesAgo);
        ...
        for (Order order : pendingOldOrders) {
            try {
                ...
                // We don't have a user to set as updatedBy for system action, so we can set it to "SYSTEM" or leave null.
                ...
                for (OrderItem item : order.getOrderItems()) {
                    ...
                    Integer updatedRows = productVariantRepository.incrementReservedQuantity(
                            variant.getVariantId(), -quantityToRelease);
                    if (updatedRows == 0) {
                        log.warn("Failed to release reserved quantity for variantId: {} (orderId: {}) - variant not found",
                                variant.getVariantId(), order.getOrderId());
                    } else {
                        log.debug("Released reserved quantity of {} for variantId: {} (orderId: {})",
                                quantityToRelease, variant.getVariantId(), order.getOrderId());
                    }
                }
