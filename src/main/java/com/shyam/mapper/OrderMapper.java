...
public class OrderMapper {
  ...
  public void updateOrderEntity(Order order, UpdateOrderRequestDTO dto) {
    ...
    if (dto.getDeliveryType() != null) order.setDeliveryType(dto.getDeliveryType());

    if (dto.getNotes() != null) order.setNotes(dto.getNotes());

    if (dto.getOrderStatus() != null) order.setOrderStatus(parseOrderStatus(dto.getOrderStatus()));

    if (dto.getPaymentMethod() != null)
      order.setPaymentMethod(parsePaymentMethod(dto.getPaymentMethod()));

    if (dto.getTotalCost() != null) order.setTotalCost(dto.getTotalCost());

    if (dto.getDueAmount() != null) order.setDueAmount(dto.getDueAmount());

    order.setPaymentStatus(calculatePaymentStatus(order.getTotalCost(), order.getDueAmount()));
  ...
  public GetOrderByIdResponseDTO mapToOrderResponseDTO(Order order) {
    ...
    return GetOrderByIdResponseDTO.builder()
        ...
        .productIds(order.getItems().stream().map(OrderItem::getProductId).toList())
  ...
  private OrderPaymentStatus calculatePaymentStatus(BigDecimal total, BigDecimal due) {
    ...
    if (due == null || due.compareTo(BigDecimal.ZERO) == 0) return OrderPaymentStatus.PAID;

    if (due.compareTo(total) < 0) return OrderPaymentStatus.PARTIALLY_PAID;
