...
public class PublicProductController {
    ...
    @Operation(summary = "Get all active products", description = "Retrieve a paginated list of ACTIVE products only.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get product by ID", description = "Get product details by ID, only if status is ACTIVE.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful retrieval",
                    content = @Content(schema = @Schema(implementation = BaseResponseDTO.class))),
            @ApiResponse(responseCode = "404", description = "Product not found or not active", content = @Content),
            @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
    })
